...
export const api = {
  ...
  createTeacher: (payload, token) =>
    request('/teachers', { method: 'POST', body: payload, token }),
  updateTeacher: (id, payload, token) =>
    request(`/teachers/${id}`, { method: 'PUT', body: payload, token }),
  deleteTeacher: (id, token) =>
    request(`/teachers/${id}`, { method: 'DELETE', token }),

  listStudents: (token, page = 1, q = '') =>
    request(`/students?page=${page}&limit=10&q=${encodeURIComponent(q)}`, { token }),
  createStudent: (payload, token) =>
    request('/students', { method: 'POST', body: payload, token }),
  updateStudent: (id, payload, token) =>
    request(`/students/${id}`, { method: 'PUT', body: payload, token }),
